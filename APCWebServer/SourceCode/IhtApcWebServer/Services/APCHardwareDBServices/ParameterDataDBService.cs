...
namespace IhtApcWebServer.Services.APCHardwareDBServices
{
	public class ParameterDataDBService : IParameterDataDBService
	{
        ...
        public async Task<ParameterDataModel?> GetDeviceParamByParamGroupAndParamIdAsync(int deviceId, ParamGroup paramGroup, int paramId, CancellationToken cancellationToken)
        {
            ...
            var entries = default(ParameterDataModel);

            try
            {
                entries = await dbContext.ParameterDatas
                    .AsNoTracking()
                    .Include(p => p.APCDevice)
                    .Include(p => p.DynParams)
                        .ThenInclude(dp => dp.ConstParams)
		...
		public async Task<ParameterDataModel?> GetEntryByAPCDeviceAndParamIdAsync(APCDeviceModel apcDevice, ParamGroup paramGroup, int paramId, CancellationToken cancellationToken)
		{
			...
			var entry = await dbContext.ParameterDatas
				.AsNoTracking()
				.Include(p => p.DynParams)
				.SingleOrDefaultAsync(p => p.DynParams != null && p.APCDevice != null && p.ParamGroupId == paramGroup && p.DynParams.ParamId == paramId && p.APCDevice.Id == apcDevice.Id, cancellationToken);
