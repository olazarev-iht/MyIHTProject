...
namespace IhtApcWebServer.Services.APCHardwareDBServices
{
	public class ConstParamsDBService : IConstParamsDBService
	{
		...
		public async Task DeleteAllEntriesAsync(CancellationToken cancellationToken, List<IhtDevice> deviceList = null)
		{
			...
			{
                var deviceNumsArr = deviceList.Select(d => d.DeviceNumber).ToArray();

                var deviceNumsStr = string.Join(",", deviceNumsArr);
                ...
                cmd += $" Where Id in (Select dyn.ConstParamsId From DynParams dyn join ParameterDatas pd " +
					...
					$" on pd.APCDEviceId = apc.Id  Where apc.Num in ({deviceNumsStr}) )";
			}
