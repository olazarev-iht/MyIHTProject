...
namespace IhtApcWebServer.Services.APCCommunic
{
    public class APCCommunicManager : ICommunicService
    {
        ...
        public IhtDevices.PasswordLevel_SW UserPasswordLevel { get; set; } = IhtDevices.PasswordLevel_SW.Level_0;
        ...
        private async Task CmdConnectExecAsync()
        {
            ...
            if (blResult)
            {
                ...
                // Fill Dyn Params DB
                var connectedDevices = _ihtDevices.GetDevices().Where(d => d.IsConnected).ToList();
                //await _parameterDataInfoManager.UpdateAPCHardwareDataAsync(CancellationToken.None, connectedDevicesAmount);
                await _parameterDataInfoManager.CopyModbusAPCHardwareDataAsync(CancellationToken.None, connectedDevices);
            }
