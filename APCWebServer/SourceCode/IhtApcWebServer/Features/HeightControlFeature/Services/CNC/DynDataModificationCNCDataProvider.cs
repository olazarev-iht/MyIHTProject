...
namespace IhtApcWebServer.Features.HeightControlFeature.Services.CNC
{
	public class DynDataModificationCNCDataProvider : IDisposable
	{
        ...
        public ParameterDataModel? ProcessHeight
        {
            get
            {
				IhtModbusParamDyn.eIdxTechnology? technologyParam = null;

                switch (CurrentParamsType)
				{
                    case CommonConstants.PRE_HEAT:
                        technologyParam = IhtModbusParamDyn.eIdxTechnology.PreHeatHeight;
						break;
                    case CommonConstants.PIERCING:
                        technologyParam = IhtModbusParamDyn.eIdxTechnology.PierceHeight;
                        break;
                    case CommonConstants.CUTTING:
                        technologyParam = IhtModbusParamDyn.eIdxTechnology.CutHeight;
                        break;
                }

				if (technologyParam == null) return null;

                var param = _parameterDataInfoManager.GetDeviceParamByParamGroupAndParamIdAsync(CurrentDeviceNumber, ParamGroup.Technology,
                    (int)technologyParam, CancellationToken.None).Result;
