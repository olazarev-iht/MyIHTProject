...
namespace SharedComponents.Helpers
{
    public class Units
    {
		...
		public static readonly string txtSec = "s";
		public static readonly string txtSm = "cm";
      ...
      return (IsUnitInch) ? Units.GetFormattedUnitInch(cuttingSpeed * Units.inchMultiplier) + " " + Units.txtInch + "/min"
        ...
        static public string GetFormattedPressureBar(double valueBar, double maxValue = 0d, string parameterFormat = "")
        {
            string result = String.Empty;
            ...
            if (string.IsNullOrWhiteSpace(parameterFormat))
            {
                if (maxValue >= 99999)
                {
                    result = String.Format("{0,0:0.0}", valueBar);
                }
                else if (maxValue >= 1000)
                {
                    result = String.Format("{0,0:0.0#}", valueBar);
            ...
            {
                result = String.Format(parameterFormat, valueBar);
            }
            return result;
        }
        ...
        static public string GetFormattedPressurePsi(double valuePsi, bool with4Digits = true)
        {
            ...
            if (with4Digits)
            {
                if (valuePsi >= 100)
                {
                    result = String.Format("{0,1:0.#}", valuePsi);
                }
                else if (valuePsi >= 10)
                {
                    result = String.Format("{0,2:0.##}", valuePsi);
                }
                else
                {
                    result = String.Format("{0,3:0.###}", valuePsi);
                }
            }
            else
            {
