  ...
  public class DataProcessInfo : INotifyPropertyChanged
    {
        ...
        public DataProvider<int>? DataProviderStatusLeds = new();
        public int StatusLeds
        {
            get { return _statusLeds; }
            set 
            { 
              int oldValue = _statusLeds;
              RaisePropertyChanged(ref _statusLeds, value); 
              if (!oldValue.Equals(value)) 
              { 
                DataProviderStatusLeds?.OnDataChanged(new ProcessInfoEventArgs<int>(oldValue, value));
              }
            }
        }
        ...
        public void IsProcessInfoChangedHandler(object? sender, PropertyChangedEventArgs eventArgs)
        {
            ...
            if (!IsProcessInfoUpdating)
            {
                if (_IAPCWorker != null)
                {
                    if (!(eventArgs?.PropertyName?.Equals("SlaveId") ?? false))
                    {
        ...
        private void SaveRowInToErrorLog(object oldValue, object newValue)
        {
            ...
            if ((oldValueInt ?? 0) == 0 && (newValueInt ?? 0) != 0)
            {
                ...
                if (description.StartsWith("Er.")) return;
        ...
        public DataProvider<bool>? DataProviderIsProcessActive = new();
        public bool IsProcessActive
        {
            get { return _isProcessActive; }
            set 
            { 
              bool oldValue = _isProcessActive;
              RaisePropertyChanged(ref _isProcessActive, value); 
              if (!oldValue.Equals(value)) 
              { 
                DataProviderIsProcessActive?.OnDataChanged(new ProcessInfoEventArgs<bool>(oldValue, value));
              }
            }
        }
        ...
        public DataProvider<bool>? DataProviderIsControlActive = new();
        public bool IsControlActive
        {
            get { return _isControlActive; }
            set 
            { 
              bool oldValue = _isControlActive;
              RaisePropertyChanged(ref _isControlActive, value); 
              if (!oldValue.Equals(value)) 
              { 
                DataProviderIsControlActive?.OnDataChanged(new ProcessInfoEventArgs<bool>(oldValue, value));
              }
            }
        }
        ...
        public DataProvider<bool>? DataProviderIsClearanceControlManual = new();
        public bool IsClearanceControlManual
        {
            get { return _isClearanceControlManual; }
            set 
            { 
              bool oldValue = _isClearanceControlManual;
              RaisePropertyChanged(ref _isClearanceControlManual, value); 
              if (!oldValue.Equals(value)) 
              { 
                DataProviderIsClearanceControlManual?.OnDataChanged(new ProcessInfoEventArgs<bool>(oldValue, value));
              }
            }
        }
        ...
        internal void UpdateDatas(IhtModbusData ihtModbusData)
        {
            ...
            {
              Console.WriteLine($"CurrOutHeatO2:{CurrOutHeatO2},  CurrOutCutO2={CurrOutCutO2},  CurrOutFuelGas={CurrOutFuelGas}");
            }
            #endif
