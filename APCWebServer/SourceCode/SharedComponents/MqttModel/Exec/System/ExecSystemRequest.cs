      ...
      else
      {
        ...
        else
        {
          ...
          if (loadTechnologyData)
          {
            ...
            int oldValue = (int)ihtDevices.DataBaseId;
            ihtDevices.DataBaseId = cuttingDataModel?.Id;
            IhtSettings.UpdateDataBaseControl(ihtDevices._systemSettings);
            IhtDevices.IsCuttingDataBaseIdChanged(new object(), new IhtData.DataProvider.ProcessInfoEventArgs<int>(oldValue, cuttingDataModel.Id));
          }
          else
          {
            // Status: Fehler beim Laden vom angeforderten Datensatz in Gerät(e)
