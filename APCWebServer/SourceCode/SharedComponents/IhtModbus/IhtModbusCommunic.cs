...
using SharedComponents.MqttModel.Exec.Common;
...
namespace SharedComponents.IhtModbus
{
  public class IhtModbusResult
    ...
    public class IhtModbusCommunic
    {
		...
		private IErrorLogDBService mainWndHlp
		{
			get
			{
				return _provider.GetService<IErrorLogDBService>()
					?? throw new Exception($"ErrorLogDBService is not available in {typeof(IhtModbusCommunic).Name}");
			}
		}
    ...
    static public UInt16 GetSlaveIdBit(int _slaveId)
        ...
        private async Task<bool> ConnectRtuAsync(ArrayList SlaveIds, IhtMsgInfo msgInfo, UInt16 u16OnSlaveIdBits, bool isRobot)
        {
            ...
            try
            {
                ...
                {
					await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Error.ToString(), "_SystemMsg_NoAvailableComPort");
					return false;
                }
				...
				var msg = "Port: {0}; Baudrate: {1}";
				await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Info.ToString(), msg, slaveId: null, comPort, communicData.Baudrate.ToString());
            ...
            else
            {
                ...
                if (find && communicData.ComPort != communicData.ComPortLast)
                {
                    ...
                    //}));
                    await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Info.ToString(), "_CultureMsgLogRecognizingTorches");

                    OpenPort(communicData.ComPortLast);

                    string msg = "Port: {0}; Baudrate: {1}";
                    await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Info.ToString(), msg, slaveId: null, communicData.ComPortLast, communicData.Baudrate.ToString());
        ...
        private async Task<bool> ConnectTcpAsync(ArrayList SlaveIds, IhtMsgInfo msgInfo, UInt16 u16OnSlaveIdBits, bool isRobot)
        {
            ...
            try
            {
                ...
                string msg = "IP-Address: {0}; Port: {1}";
                await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Info.ToString(), msg, null, communicData.IpAddress, communicData.IpPort.ToString());
        ...
        private async Task<ushort[]> ModbusMaster_ReadHoldingRegistersAsync(byte slaveAddress, ushort startAddress, ushort numRegisters, IhtModbusResult ihtModbusResult)
        {
            ...
            {
                ihtModbusResult.Result = false;
                await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Exception.ToString(), exc.Message, slaveAddress);
        ...
        private ushort[] ReadSimulationData(int slaveAddress, ushort startAddress, ushort numRegisters)
        {
            ...
            {
                mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Exception.ToString(), exc.Message, slaveAddress);
            }
            return au16Data;
        }
        ...
        /// 
        /// </summary>
        class FirstConnectedFwVersion 
        {
          public bool Valid { get; set; }
          public int FwVersion { get; set; }
          public int SlaveId { get; set; }
          public bool IsFwVersionEqual(int fwVersion)
          {
            return FwVersion == fwVersion;
          }
          public IhtModbusData GetData(IhtModbusCommunic ihtModbusCommunic)
          {
            return ihtModbusCommunic.GetData(SlaveId);
          }
        }
        ...
        {
          #if DEBUG
            Debug.Print($"Start ConnectRdDataAsync: {DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss,FFF")}");
          #endif
            ...
            try
            {
                string connectingTorch = "_CultureMsgLogConnectingTorch";
                string torch = "_CultureTorch";
                string torchType = "_CultureTorchType";
                string txtSlaveId = "_CultureSlaveId";
                string notConnected = "_CultureNotConnected";
                string connected = "_CultureConnected";
                string fwSpecialActive = "_CultureMsgLogFwSpecialActive";
                string fwUpdateRequired = "_CultureMsgLogFwUpdateRequired";
                string torchWrongIdentity = "_CultureTorchTypeWrongIdentity";
                string calibrationInvalid = "_CultureCalibrationInvalid";
                string pressureOutputsLocked = "_CultureInfoPressureOutputsLocked";
                string resetAllDevices = "_CultureResetAllDevices";
                ...
                if (isExecReset)
                {
                    await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Info.ToString(), resetAllDevices);
                ...
                #region foreach
                FirstConnectedFwVersion firstConnectedFwVersion = new();
                ...
                foreach (int _slaveId in SlaveIds)
                {
                    ...
                    if (!blSlaveIdBroadcast)
                    {
                        ...
                        msg = connectingTorch;
                        await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Info.ToString(), msg, currSlaveId);
                    }
                    ...
                    if (!ihtModbusResult.Result)
                    {
                        if (!blSlaveIdBroadcast)
                        {
                            await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Warning.ToString(), notConnected, currSlaveId);
                        ...
                        if (!blSlaveIdBroadcast && !IsSimulation)
                        {
                            ...
                            if (isApplFwMissing)
                            {
                                ...
                                if (isDownloadkernelActive)
                                {
                                    ...
                                    await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Error.ToString(), "_CultureFirmwareIsMissing", currSlaveId);
                                    await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Warning.ToString(), "_CultureContactService");
                                }
                                // Port wieder öffnen
                                OpenPort();
                                // Und gleich zum Nächsten
                    ...
                    // FW-Version für konstante Parameter kopieren
                    int fwVersion =  (ihtModbusData.GetValueDeviceInfo(IhtModbusParamConst.eIdxDeviceInfo.FwVersion) << 8)
                                   + (ihtModbusData.GetValueDeviceInfo(IhtModbusParamConst.eIdxDeviceInfo.FwSubVersion) & 0xFF);
                    // Konstante Parameter kopieren
                    if (ihtModbusResult.Result && firstConnectedFwVersion.Valid && firstConnectedFwVersion.IsFwVersionEqual(fwVersion))
                    {
                      IhtModbusData ihtModbusDataSrc = firstConnectedFwVersion.GetData(this);
                      // Technologie-Parameter Const. kopieren
                      ihtDevices.Copy_TechnologyConst(ihtModbusDataSrc, ihtModbusData);
                      // Process-Parameter Const. kopieren
                      ihtDevices.Copy_ProcessConst(ihtModbusDataSrc, ihtModbusData);
                      // Config-Parameter Const. kopieren                                                                                     
                      ihtDevices.Copy_ConfigConst(ihtModbusDataSrc, ihtModbusData);
                      // Sevice-Parameter Const. kopieren                                                                                    
                      ihtDevices.Copy_ServiceConst(ihtModbusDataSrc, ihtModbusData);
                    }
                    // Konstante Parameter auslesen
                    else
                    {
                    ...
                    // Dynamische Parameter kopieren
                    else if (ihtModbusResult.Result)
                    {
                      IhtModbusData ihtModbusDataSrc = firstConnectedFwVersion.GetData(this);
                      // Technologie-Parameter Dyn. koieren                                                                                 
                      ihtDevices.Copy_TechnologyDyn(ihtModbusDataSrc, ihtModbusData);
                      // Process-Parameter Dyn. koieren                                                                                     
                      ihtDevices.Copy_ProcessDyn(ihtModbusDataSrc, ihtModbusData);
                      // Config-Parameter Dyn. koieren                                                                                      
                      ihtDevices.Copy_ConfigDyn(ihtModbusDataSrc, ihtModbusData);
                      // Sevice-Parameter Dyn. koieren                                                                                      
                      ihtDevices.Copy_ServiceDyn(ihtModbusDataSrc, ihtModbusData);
                    }
                    ...
                    if (!ihtModbusResult.Result)
                    {
                        if (!blSlaveIdBroadcast)
                        {
                            ...
                            await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Warning.ToString(), notConnected, (int)(currSlaveId - SlaveId.Id_Default));
                        }
                        continue;
                    }
                    ...
                    if (!blSlaveIdBroadcast)
                    {
                        ...
                        string torchNumber = currSlaveId.ToString();
                        ...
                        await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Ok.ToString(), "{0}; {1}; {2}", currSlaveId, connected, serialNo, partNo);
                        ...
                        await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Info.ToString(), "{0}, {1}", currSlaveId, fwVersionCU, hwVersionCU);
                        ...
                        if (!firstConnectedFwVersion.Valid)
                        {
                          firstConnectedFwVersion.Valid = true;
                          firstConnectedFwVersion.FwVersion = fwVersionCuValue;
                          firstConnectedFwVersion.SlaveId = firstConnectedSlaveId;
                        }
                        ...
                        if (((u16FwVersionCU & 0xFF00) >> 8) >= 90)
                        {
                            ...
                            msg = "CU+ {0}: {1}";
                            await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Warning.ToString(), msg, currSlaveId, fwSpecialActive, val);
                        }
                        else
                        {
                        ...
                        if (!blFwSpecialCU && fwVersionCuValue < fwMinimumVersion)
                        {
                            ...
                            msg = "CU+ {0}: {1}";
                            await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Warning.ToString(), msg, currSlaveId, fwUpdateRequired, val);
                        }
                        else
                        {
                        ...
                        {
                            msg = "{0}; {1}";
                            await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Warning.ToString(), msg, currSlaveId, fwVersionTorch, hwVersionTorch);
                        }
                        ...
                        if (!IsSimulation && !blFwSpecialTorch && fwVersionTorchValue < IhtDevices.TorchFwMinimumVersion)
                        {
                            if (fwVersionTorchValue == 0)
                            {
                                ...
                                msg = "FIT+3 {0}";
                                await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Error.ToString(), msg, currSlaveId, notConnected);
                            }
                            else
                            {
                                // FW-Update Torch
                        ...
                        string[] fwTorchType = new[] { ihtModbusData.GetDeviceInfoValue(IhtModbusData.DataDeviceInfoValue.TorchType) };
                        string fwMessage = string.Empty;
                            ...
                            && fwVersionTorchValue >= IhtDevices.TorchAvailableStatusInfoFwVersion)
                        {
                            ...
                            if (await ihtDevices.Read_StatusInfoSpecificAsync(ihtModbusData, dataOffsetAddr, dataCounts) == true)
                            {
                                ...
                                if (idx < listTblData.Count)
                                {
                                    ...
                                    (fwMessage, fwTorchType) = ihtModbusData.GetTorchTypedIdxInfo_forErrorLog(torchTypeIdx);
                                }
                                ...
                                if (torchTypeIdx == -1)
                                {
                                    ...
                                    if (idx < listTblData.Count && statusReg1Length >= 8)
                                    {
                                        ...
                                        switch (bits)
                                        {
                                            ...
                                            case 1:
                                                (fwMessage, fwTorchType) = ihtModbusData.GetTorchControllerNotConnected();
                                                break;
                                            // Bit0=0, Bit1=1: Torch Not Defined
                                            case 2:
                                                (fwMessage, fwTorchType) = ihtModbusData.GetTorchTypeUndefined();
                                                break;
                                            // Bit0=1, Bit1=1: Torch Code Fault
                                            case 3:
                                                (fwMessage, fwTorchType) = ihtModbusData.GetTorchTypeCodeFault();
                                                break;
                                        }
                                    }
                                }
                        ...
                        else
                        {
                            ...
                            string msgUpper = fwTorchType[0].ToUpper();
                        ...
                        if (fwVersionTorchValue > 0)
                        {
                            //await mainWndHlp.AddEntryByItemsValAsync(blWarning ? IhtMsgLog.Info.Warning.ToString() : IhtMsgLog.Info.Info.ToString(), msg);
                            var newParams = fwTorchType.ToList();
                            newParams.Add(torchPartNo);

                            //TODO: Test it very well!!!!!
                            await mainWndHlp.AddEntryByItemsValAsync(blWarning ? IhtMsgLog.Info.Warning.ToString() : IhtMsgLog.Info.Info.ToString(),
                                fwMessage, currSlaveId, newParams.ToArray());
                        }
                        ...
                        if (blWrogTorchType)
                        {
							...
							await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Warning.ToString(), torchWrongIdentity, currSlaveId);
							blWarning = true;
                            blWarningTorchType = true;
                        }
                        ...
                        if (dataCmdExecution != null && dataCmdExecution.IsLockPressureOutput)
                        {
							...
							await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Warning.ToString(), pressureOutputsLocked, currSlaveId);
						}
                ...
                if (isRobot)
                {
                    ...
                    if (isCuPartNo_100684 || isCuPartNo_101189)
                    {
						...
						await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Warning.ToString(), "Control units types other than (PartNo: 100685) are not permitted!");
						await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Warning.ToString(), "_CultureContactService");
					}
                }
                ...
                else if (isCuPartNo_100684 && isCuPartNo_101189)
                {
					...
					//mainWndHlp.SetStatusMsg(IhtMsgLog.Info.Warning, "Please contact the service.");
					await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Warning.ToString(), "Different control units types (PartNo: 101189, 100684) are not permitted!");
					await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Warning.ToString(), "_CultureContactService");
				}
            }
            catch (Exception exc)
            {
				...
				await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Exception.ToString(), exc.Message);
			}
          #if DEBUG
            Debug.Print($"End ConnectRdDataAsync: {DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss,FFF")}");
          #endif
        }
        ...
        private async Task<bool> IdentifyOverDownloadkernel(IhtCommunic.SlaveId.Id slaveId)
        {
            ...
            try
            {
                ...
                if (!errorInfo.IsResult())
                {
					...
					await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Error.ToString(), errorInfo.Message);
				}
                ...
                if (errorInfo.IsResult())
                {
                    ...
                    else
                    {
						...
						await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Error.ToString(), errorInfoRead.Message);
					}
                }
                else
                {
                    ...
                    if (errorInfoRead.IsResult())
                    {
                        ...
                        foreach (string s in vs)
                        {
                            if (s.Contains(txtApplNotAvailable))
                            {
								...
								await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Error.ToString(), s);
							}
                            else
                            {
								await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Info.ToString(), s);
							}
                        }
                    }
                    else
            ...
            catch (Exception ex)
            {
				...
				await mainWndHlp.AddEntryByItemsValAsync(IhtMsgLog.Info.Exception.ToString(), ex.Message);
